...
namespace GoogleARCore.Examples.HelloAR
{
    ...
    public class HelloARController : MonoBehaviour
    {
        ...
        public CatalogObject CatalogActiveObject;
        public CatalogObject CurrentCatalogObject;
        ...
        /// <summary>
        /// List of placed models on world
        /// </summary>
        ...
        private void ChangeObjectMaterial(GameObject model, Color color)
        {
            //CHANGE MATERIAL TO SELECTED
            List<Material> mats = new List<Material>();
            MeshRenderer[] objectMats = model.transform.GetComponentsInChildren<MeshRenderer>();

            for (int i = 0; i < objectMats.Length; i++)
            {
                for (int o = 0; o < objectMats[i].materials.Length; o++)
                {
                    mats.Add(objectMats[i].materials[o]);
                }
            }
        ...
        public void ConfirmPosition()
        {
            if(IsReplacing)
            {
                ChangeObjectMaterial(SelectedObject, DefaultColor);
            }
            else
            {
                ChangeObjectMaterial(placedModels[placedModels.Count - 1], DefaultColor);
            }
            IsPlacing = false;
            IsReplacing = false;
        ...
        public void DeleteObject()
        {
            ...
            {
                int number = 0;
                for (int i = 0; i < placedModels.Count; i++)
                {
                    if(placedModels[i] == SelectedObject)
                    {
                        Destroy(placedModels[i]);
                        number = i;
                    }
                }
                placedModels.RemoveAt(number);
                IsReplacing = false;
            }
            else
            {
                Destroy(placedModels[placedModels.Count - 1]);
